...
class Tasks:
    ...
    @staticmethod
    def flow_edmonds_karp(g, source, sink):
        """
            Task 5 Edmonds-Karp algorithm: max flow in the network
            Graph: network
        """
        capacities = func.convert_adjmap_to_adjmatrix(g, flow=True)
        flow = copy.deepcopy(capacities)
        for item in flow:
            for _i in flow[item]:
                if flow[item][_i] is not None:
                    flow[item][_i] = 0
        return func.edmonds_karp(capacities, flow, source, sink)
